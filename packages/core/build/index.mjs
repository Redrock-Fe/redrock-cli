      ...
      [
        ...
        },
        {
          ...
          choices: PACKAGE_MANAGERS.map((packageManager2) => {
            ...
            return {
              title: packageManagerColor(packageManager2.display || packageManager2.name),
...
Scaffolding project in ${root}...`);
  ...
  const doc = yaml.load(fs.readFileSync(path.join(templateDir, ".gitlab-ci.yml"), "utf-8"));
...
program.command("create [name]").description("Use redrock-cli to create a new project").action((name) => {
  if (name) {
