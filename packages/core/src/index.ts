...
import {formatTargetDir,isEmpty,copy,pkgFromUserAgent,emptyDir,isValidPackageName,toValidPackageName} from "./utils/index"
...
async function init(projectName:string) {
  ...
  let result: prompts.Answers<
    'projectName' | 'overwrite' | 'packageName' | 'framework' | 'variant' | 'packageManager'
  >
  try {
      ...
      [
        ...
        },
        {
            ...
            choices: PACKAGE_MANAGERS.map((packageManager) => {
               ...
               return {
                title:packageManagerColor(packageManager.display || packageManager.name),
  ...
  const doc:any = yaml.load(fs.readFileSync(path.join(templateDir,'.gitlab-ci.yml'),'utf-8'))
